...
[`samples-controls`](https://github.com/abap2UI5/samples-controls) (plus optionally
`linter` and `mcp-server` itself). Clone them into one folder and point
...
server and prefers the local `mcp-server` checkout over downloading via npx.
...
mcp-server is deliberately system-less. The extension additionally offers
